...
namespace PacketSniffer
{
    public class TCPHeader
    {
        ...
        private short sChecksum = 555;                  //Sixteen bits for the checksum (checksum can be negative so taken as short)
        ...
        public TCPHeader(byte[] byBuffer, int nReceived)
        {
            ...
            {
                MessageBox.Show(ex.Message, "PacketSniffer (TCP)" + (nReceived), MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        public string AcknowledgementNumber
        {
            get
            {
                //If the ACK flag is set then only we have a valid value in the acknowlegement field, so check for it beore returning anything
        ...
        public string UrgentPointer
        {
            get
            {
                //If the URG flag is set then only we have a valid value in the urgent pointer field, so check for it beore returning anything
