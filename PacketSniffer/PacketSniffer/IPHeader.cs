...
namespace PacketSniffer
{
    public class IPHeader
    {
        ...
        private short sChecksum;                    //Sixteen bits containing the checksum of the header (checksum can be negative so taken as short)
        ...
        public IPHeader(byte[] byBuffer, int nReceived)
        {
            ...
            try
            {
                ...
                //The last four bits of the version and header length field contain the header length, we perform some simple binary airthmatic operations to extract them
                byHeaderLength <<= 4;
                byHeaderLength >>= 4;
                //Multiply by four to get the exact header length
                byHeaderLength *= 4;
                //Copy the data carried by the data gram into another array so that according to the protocol being carried in the IP datagram, start copying from the end of the header
                Array.Copy(byBuffer, byHeaderLength, byIPData, 0, usTotalLength - byHeaderLength);
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message, "PacketSniffer (IP)", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        public string Flags
        {
            get
            {
                //The first three bits of the flags and fragmentation field represent the flags (which indicate whether the data is fragmented or not)
        ...
        public string FragmentationOffset
        {
            get
            {
                //The last thirteen bits of the flags and fragmentation field contain the fragmentation offset
